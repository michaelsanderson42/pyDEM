...
def get_fn(elev, name=None):
    ...
    """
    if not elev.coords('latitude').has_bounds():
        elev.coords('latitude').guess_bounds()
    if not elev.coords('longitude').has_bounds():
        elev.coords('longitude').guess_bounds()

    LLC_lat = np.min(elev.coords('latitude').bounds)
    URC_lat = np.max(elev.coords('latitude').bounds)
    LLC_lon = np.min(elev.coords('longitude').bounds)
    URC_lon = np.max(elev.coords('longitude').bounds)
    coords = [LLC_lat, LLC_lon, URC_lat, URC_lon]
...
def get_fn_from_coords(coords, name=None):
    ...
    NS2 = ["S", "N"][coords[2] > 0]
    EW2 = ["W", "E"][coords[3] > 0]
    new_name = "{}{:.3g}{}{:.3g}_{}{:.3g}{}{:.3g}".format( \
        NS1, coords[0], EW1, coords[1], NS2, coords[2], EW2, coords[3])
    if name is not None:
        new_name += '_' + name

    return new_name.replace('.', 'o') + '.nc'


def mk_dx_dy_from_latlon(lat, lon):
    """
    Extracts the change in x and y coordinates from the latitude and longitudes supplied.
    Uses the Haversine formula to calculate the distances.
    """
    dX = np.zeros((lat.shape[0]-1))
    for j in list(range(len(dX))):
        dX[j] = haversine((lat[j+1], lon[1]), (lat[j+1], lon[0])) * 1000  # km2m
    dY = np.zeros((lat.shape[0]-1))
    for i in list(range(len(dY))):
        dY[i] = haversine((lat[i], lon[0]), (lat[i+1], lon[0])) * 1000  # km2m
